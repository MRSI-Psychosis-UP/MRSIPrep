...
These bodies are kept self-contained (imports live inside each function) so they
also survive Nipype's ``Function`` source serialization if ever wrapped that way;
the shipped nodes use :class:`~mrsiprep.workflows.nipype_engine.adapters.StepInterface`.
...
def step_tissue_seg(config, subject, session, ctx):
    ...
    t1_path, precomputed_tissue_t1, p3_override, brain_mask_override = _step_tissue_segmentation(
        config, subject, session, ctx["raw_t1"], ctx["t1_path"], Debug(verbose=config.verbose)
    )
    ctx = dict(ctx)
    ctx.update(
        t1_path=t1_path,
...
def step_anat(config, subject, session, ctx):
    ...
    ctx = dict(ctx)
    ctx["anat"] = _step_anatomical_prep(
        config, subject, session, ctx["t1_path"], ctx["p3_override"], ctx["brain_mask_override"], Debug(verbose=config.verbose)
    )
    return ctx
...
def step_mrsi(config, subject, session, ctx):
    ...
    mrsi, qc_report_mrsi_preproc = _step_mrsi_preprocessing(config, subject, session, ctx["inputs"], Debug(verbose=config.verbose))
    ctx = dict(ctx)
...
def step_tissue_probmaps(config, subject, session, ctx):
    ...
    ctx["tissue"] = _step_tissue_probmaps(
        config, subject, session, ctx["anat"], ctx["mrsi"], ctx["registration"], ctx["precomputed_tissue_t1"], Debug(verbose=config.verbose)
    )
    return ctx
...
def step_tissue_qc(config, subject, session, ctx):
    ...
    dseg_for_qc = (
        synthseg_native_labels_path(config, subject, session)
        if config.processing_mode == "parc-con" and config.tissue_backend == "synthseg-fast"
        else None
    )
    tissue = ctx["tissue"]
    ctx = dict(ctx)
...
def step_pvc(config, subject, session, ctx):
    ...
    corrected_maps, tissue_4d = _step_pvc(config, subject, session, ctx["mrsi"], ctx["tissue"], Debug(verbose=config.verbose))
    ctx = dict(ctx)
...
def step_resampling(config, subject, session, ctx):
    ...
    transformed, qc_report_registration = _step_resampling(
        config, subject, session, ctx["anat"], ctx["mrsi"], ctx["registration"], ctx["corrected_maps"], ctx["raw_t1"], Debug(verbose=config.verbose)
    )
    ctx = dict(ctx)
...
def step_synthseg_parc_qc(config, subject, session, ctx):
    ...
    preliminary_parcels, parcel_qc = _step_synthseg_parcellation_qc(
        config, subject, session, ctx["raw_t1"], ctx["mrsi"], ctx["registration"], Debug(verbose=config.verbose)
    )
    ctx = dict(ctx)
...
def step_parcellation(config, subject, session, ctx):
    ...
    parcels, qc_report_parcellation = _step_parcellation(
        config, subject, session, ctx["raw_t1"], ctx["mrsi"], ctx["anat"], ctx["registration"], ctx["preliminary_parcels"], Debug(verbose=config.verbose)
    )
    ctx = dict(ctx)
...
def step_regional(config, subject, session, ctx):
    ...
    ctx = dict(ctx)
    ctx["regional"] = _step_regional_extraction(
        config, subject, session, ctx["corrected_maps"], ctx["parcels"], ctx["mrsi"], ctx["tissue"], Debug(verbose=config.verbose)
    )
    return ctx
...
def step_connectivity(config, subject, session, ctx):
    ...
    connectivity, qc_report_connectivity = _step_connectivity(
        config, subject, session, ctx["regional"], ctx["parcels"], ctx["corrected_maps"], ctx["mrsi"], ctx["tissue"], Debug(verbose=config.verbose)
    )
    ctx = dict(ctx)
...
def step_reports(config, subject, session, ctx):
    ...
    outputs = _step_reports(config, subject, session, outputs, Debug(verbose=config.verbose))
