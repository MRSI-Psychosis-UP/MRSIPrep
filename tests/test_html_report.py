...
class GenerateSubjectReportTabsTests(unittest.TestCase):
    ...
    def test_connectivity_tab_present_only_when_sections_given(self):
        """Gated on qc_sections['connectivity'] being non-None, not directly
        on config.write_connectivity -- a run that produced connectivity
        sections without --write-connectivity still gets a tab explaining the
        matrix wasn't requested."""
        with tempfile.TemporaryDirectory() as td:
